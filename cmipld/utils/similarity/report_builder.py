...
def _issue_link_suffix() -> str:
    """
    Build a "from [Issue NNN](url)" suffix for the report footer using GitHub
    Actions environment variables. Returns an empty string when no issue
    context is available (e.g. running ReportBuilder from a notebook), so
    the footer degrades gracefully.

    Inputs (all from os.environ):
      ISSUE_NUMBER       — set by new_issue.py main() and also by Actions
      GITHUB_SERVER_URL  — e.g. https://github.com (always set in Actions)
      GITHUB_REPOSITORY  — e.g. WCRP-CMIP/Essential-Model-Documentation
    """
    import os
    ...
    if server and repo:
        return f" from [Issue {issue_num}]({server}/{repo}/issues/{issue_num})"
    # Fallback when only ISSUE_NUMBER is available — preserve provenance even
    # without a clickable link (rare, but happens during local testing).
    return f" from Issue {issue_num}"
...
class ReportBuilder:
    ...
    def _footer(self) -> str:
        # The trailing "from [Issue NNN](url)" suffix comes from the env-var
        # helper above. Sits inside the same italics span as the rest of the
        # footer so the line reads as one continuous credit/timestamp/source.
        return (
            "---\n"
            "_Generated by [cmipld](https://github.com/WCRP-CMIP/CMIP-LD) · "
            f"{_now()}{_issue_link_suffix()}_\n"
        )
...
def build_subgrid_report(data: dict, folder_items: list | None = None) -> str:
    ...
    lines.append(
        "---\n"
        f"_Generated by [cmipld](https://github.com/WCRP-CMIP/CMIP-LD) · {_now()}{_issue_link_suffix()}_\n"
    )
