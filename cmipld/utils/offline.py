...
class LD_server:
    ...
    def create_temp_dir(self):
        ...
        if not self.temp_dir:
            self.temp_dir = tempfile.TemporaryDirectory(prefix='cmipld_local_',suffix=datetime.datetime.now().isoformat().split('.')[0])
    ...
    def clone_repos(self, repos: List[Tuple[str, str]], branch="production", dir='src-data', override = 'n'):
        ...
        for repo_url, target_name in repos:
            ...
            if os.path.exists(repo_path):
                if override != 'y':
                    override = input(f"Repo '{target_name}' already exists. Delete and replace? (y/n): ").lower()
            ...
            clone = os.popen( ' '.join(["git", "clone", "--branch", branch, "--single-branch", repo_url, repo_path])).read()
            print(clone)
    ...
    def copy_existing_repos(self, repo_paths: List[str], override = 'n'):
        ...
        for tocopy in repo_paths:
            ...
            print('Copying the repo into LocalServer ', repo_path,'-->', repo_name)
    ...
    def rollback_repo(self, repo_name: str, commit_hash: str):
        ...
        if not os.path.exists(repo_path):
            raise FileNotFoundError(f"Repository '{repo_name}' not found in {temp_dir}")
