...
class JSONSimilarityFingerprint:
    ...
    def export_similar_pairs(self, threshold: float = 0.85, group: bool = False) -> Union[List[str], Dict[int, List[str]]]:
        ...
        Args:
            threshold: Similarity score 0-1 (default 0.85 = very similar)
            group: If False (default), return list of pair strings
                   If True, return dict of grouped similar files
        
        Returns:
            If group=False:
                ["file1.json ↔ file2.json (98%)", ...] sorted by similarity
            
            If group=True:
                {0: ["file1.json", "file2.json", "file3.json"], 
                 1: ["file4.json", "file5.json"], ...}
                Groups files that are transitively similar
                
        Examples:
            # List of pair strings
            pairs = fp.export_similar_pairs(threshold=0.85)
            for pair in pairs:
                print(pair)
            
            # Grouped similar files
            groups = fp.export_similar_pairs(threshold=0.85, group=True)
            for group_id, files in groups.items():
                print(f"Group {group_id}: {files}")
        """
        if self.similarity_matrix is None:
            raise ValueError("Must call compute_similarity() first")
        
        if not group:
            # Return list of pair strings
            pairs_str = []
            for i in range(len(self.similarity_matrix)):
            ...
            # Union-Find to group transitive relationships
            parent = {}
            ...
            # Sort files within each group and return only groups with multiple files
            result = {}
            group_id = 0
    ...
    def to_json(self, threshold_pairs: float = 0.85, n_clusters: int = 5, threshold_outliers: float = 0.5, 
        ...
        Args:
            threshold_pairs: Similarity threshold for pairs (default 0.85)
            n_clusters: Number of clusters (default 5)
            threshold_outliers: Outlier threshold (default 0.5)
            output_file: Optional file path to save JSON (default: None = return dict)
        ...
        Returns:
            Dict with structure:
            {
                ...
                "similar_pairs": {
                    "string_format": ["file1 ↔ file2 (98%)", ...],
                    "grouped": {0: ["file1", "file2", "file3"], ...}
                },
                "clusters": {
                    0: ["file1", "file2", "file3"],
                    1: ["file4", "file5"],
                ...
                "outliers": ["file6", "file7", ...]
            }
        
        Examples:
            # Return as dict
            results = fp.to_json()
            print(results['similar_pairs']['grouped'])
...
def diff_jsons(data1: dict, data2: dict, name1: str = "File 1", name2: str = "File 2", max_depth: int = 10) -> str:
    ...
    Shows only differences, formatted as markdown.
    """
    lines = []
